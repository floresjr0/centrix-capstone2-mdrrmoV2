...
// ── Announcements JSON for modal (needed by the announcement detail modal UI) ──
...
<div class="sidebar-drawer" id="sidebarDrawer">
  <div class="drawer-brand">
    <div class="drawer-brand-gloss"></div>
  ...
  <div class="drawer-footer">
    ...
    <div class="drawer-partner-logos">
      <div class="drawer-partner-logo">
        <img src="../img/mdrrmo.png" alt="MDRRMO" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
        <svg viewBox="0 0 24 24" style="display:none"><path d="M12 2L3 7v5c0 5.25 3.75 10.15 9 11.35C17.25 22.15 21 17.25 21 12V7L12 2z"/></svg>
      </div>
      <div class="drawer-partner-logo">
        <img src="../img/basc_logo.jpg" alt="BASC" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
        <svg viewBox="0 0 24 24" style="display:none"><path d="M12 3L1 9l11 6 9-4.91V17h2V9L12 3zM5 13.18v4L12 21l7-3.82v-4L12 17l-7-3.82z"/></svg>
      </div>
      <div class="drawer-partner-logo">
        <img src="../img/ics_logo.jpg" alt="ICS" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
        <svg viewBox="0 0 24 24" style="display:none"><path d="M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4z"/></svg>
      </div>
    </div>
    <div class="drawer-partner-credit">
      &copy; <?php echo date('Y'); ?> MDRRMOxBASC_ICS. All rights reserved.
    </div>
  </div>
</div>
...
<!-- ══════════════════════════════════════════════════════
     ANNOUNCEMENT DETAIL MODAL — NEW FEATURE
     ══════════════════════════════════════════════════════ -->
  ...
  <div class="annmodal-sheet" id="annModalSheet">
    ...
    <div class="annmodal-head" id="annModalHead">
      ...
      <div class="annmodal-head-row">
        <div class="annmodal-head-icon" id="annModalIcon">
        <!-- default icon; replaced by JS on open -->
    ...
    <!-- Scroll hint bar (shown when body overflows) -->
...
<div class="mobile-shell">
  ...
  <div class="page-scroll">
    ...
    <?php if ($weather): ?>
    <div class="weather-card anim-in" style="box-shadow:0 20px 60px <?php echo $wx_colors[3]; ?>,0 8px 20px rgba(0,0,0,.12);">
      <div class="weather-banner" style="background:linear-gradient(150deg,<?php echo $wx_colors[0]; ?> 0%,<?php echo $wx_colors[1]; ?> 48%,<?php echo $wx_colors[2]; ?> 100%);">
        <div class="wx-texture"></div>
        <div class="wx-rim"></div>
    ...
    <div class="ann-list anim-in">
      ...
      <div class="ann-item" onclick="openAnnModal(<?php echo $idx; ?>)" role="button" aria-label="Basahin ang anunsyo">
        ...
        <svg viewBox="0 0 24 24" style="width:15px;height:15px;fill:var(--muted);flex-shrink:0;margin-top:3px;opacity:.5">
...
<div class="desktop-wrapper">
  ...
  <div class="desktop-content"><div class="desktop-grid"><div class="desktop-col-left"><div class="desktop-card"><div class="desktop-card-header"><h2>Current Status</h2></div><div class="desktop-card-body desktop-alert-wrap"><?php if ($activeDisaster): ?><div class="alert-banner alert-typhoon" onclick="openDisasterModal()" role="button"><div class="shimmer-stripe"></div><div class="alert-icon">⚠️</div><div class="alert-text"><div class="alert-title"><?php echo htmlspecialchars(ucfirst($activeDisaster['type'])); ?> Signal#<?php echo (int)$activeDisaster['level']; ?> — Aktibo</div><div class="alert-sub"><?php $lvlLabel=['1'=>'Low','2'=>'Moderate','3'=>'High','4'=>'Severe']; echo ($lvlLabel[(string)(int)$activeDisaster['level']]??'Moderate').' na antas ng panganib'; ?></div></div><div class="alert-pulse-wrap"><div class="alert-pulse-dot"></div></div></div><?php if ($advice): ?><div class="desktop-readybag" onclick="openReadyBagModal()"><div class="desktop-readybag-icon">🎒</div><div><div class="desktop-readybag-title">Payo sa Ready Bag <small style="font-weight:400;color:#bcaaa4;font-size:.60rem;">· Pindutin para makita</small></div><div class="desktop-readybag-text"><?php echo htmlspecialchars(mb_substr($advice['message'],0,100)); ?>…</div></div></div><?php endif; ?><?php elseif ($weather && ($weather['level']==='high'||$weather['level']==='extreme')): ?><div class="alert-banner alert-level-3"><div class="alert-icon">🌡️</div><div class="alert-text"><div class="alert-title">ALERTO SA INIT — Heat Index: <?php echo round($weather['heat_index']); ?>°C</div><div class="alert-sub">Uminom ng maraming tubig at iwasang lumabas</div></div><div class="alert-pulse-wrap"><div class="alert-pulse-dot"></div></div></div><?php if ($advice): ?><div class="desktop-readybag" onclick="openReadyBagModal()"><div class="desktop-readybag-icon">🎒</div><div><div class="desktop-readybag-title">Payo sa Ready Bag <small style="font-weight:400;color:#bcaaa4;font-size:.60rem;">· Pindutin para makita</small></div><div class="desktop-readybag-text"><?php echo htmlspecialchars(mb_substr($advice['message'],0,100)); ?>…</div></div></div><?php endif; ?><?php else: ?><div class="alert-banner alert-none"><div class="alert-icon">✅</div><div class="alert-text"><div class="alert-title">Walang aktibong sakuna sa ngayon</div><div class="alert-sub">Manatiling handa at subaybayan ang mga update</div></div></div><?php endif; ?></div></div><div class="desktop-card"><div class="desktop-card-header"><h2>Weather Forecast</h2><a href="#">Live Data</a></div><?php if ($weather): ?><div class="weather-card" style="margin:0;border-radius:0;box-shadow:none;"><div class="weather-banner" style="background:linear-gradient(150deg,<?php echo $wx_colors[0]; ?> 0%,<?php echo $wx_colors[1]; ?> 48%,<?php echo $wx_colors[2]; ?> 100%);"><div class="wx-texture"></div><div class="wx-rim"></div><div class="weather-top-row"><div class="weather-left"><div class="weather-temp-big"><?php echo round($weather['temp_c']); ?><sup>°C</sup></div><div class="weather-place-name">San Ildefonso, Bulacan</div><div class="weather-condition-label"><?php echo htmlspecialchars($weather['condition_text']); ?></div></div><div class="weather-risk-pill <?php echo $weather['level']; ?>"><?php $riskLabels=['low'=>'LOW','medium'=>'MODERATE','high'=>'HIGH','extreme'=>'SEVERE']; echo $riskLabels[$weather['level']]??strtoupper($weather['level']); ?> RISK</div></div><div class="weather-mascot-wrap" style="width:144px;height:144px;bottom:-10px;right:18px;"><span class="mascot-note"><?php echo $wx_ptcls[0]; ?></span><span class="mascot-note" style="animation-delay:.9s"><?php echo $wx_ptcls[1]; ?></span><span class="mascot-note" style="animation-delay:1.6s"><?php echo $wx_ptcls[2]; ?></span><?php echo wx_mascot_html($wx_cat, $isNightTime, 'd'); ?></div></div><div class="weather-stats-strip" style="background:linear-gradient(190deg,<?php echo $wx_colors[1]; ?> 0%,<?php echo $wx_colors[2]; ?> 100%);"><div class="w-stat-pill"><div class="stat-icon-3d"><svg viewBox="0 0 40 40"><defs><radialGradient id="dDropBody" cx="38%" cy="28%" r="65%"><stop offset="0%" stop-color="#b3e5fc"/><stop offset="35%" stop-color="#29b6f6"/><stop offset="75%" stop-color="#0277bd"/><stop offset="100%" stop-color="#01579b"/></radialGradient><radialGradient id="dDropSpec" cx="30%" cy="22%" r="35%"><stop offset="0%" stop-color="rgba(255,255,255,.85)"/><stop offset="100%" stop-color="rgba(255,255,255,0)"/></radialGradient><filter id="dDropBlur"><feGaussianBlur stdDeviation="1.2"/></filter></defs><ellipse cx="20" cy="36" rx="8" ry="3" fill="rgba(2,119,189,.22)" filter="url(#dDropBlur)"/><path d="M20 4 Q27 14 28 22 A8 8 0 0 1 12 22 Q13 14 20 4Z" fill="url(#dDropBody)"/><path d="M12.4 22 Q12 18 15 13" stroke="rgba(100,210,255,.5)" stroke-width="1.5" fill="none" stroke-linecap="round"/><path d="M20 4 Q27 14 28 22 A8 8 0 0 1 12 22 Q13 14 20 4Z" fill="url(#dDropSpec)"/><ellipse cx="17" cy="16" rx="3.5" ry="4.5" fill="rgba(255,255,255,.30)" transform="rotate(-15 17 16)"/></svg></div><div class="stat-info"><div class="stat-val"><?php echo $weather['humidity']; ?>%</div><div class="stat-label">Humidity</div></div></div><div class="w-stat-pill"><div class="stat-icon-3d"><svg viewBox="0 0 40 40"><defs><linearGradient id="dThermTube" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stop-color="#b0bec5"/><stop offset="30%" stop-color="#eceff1"/><stop offset="60%" stop-color="#cfd8dc"/><stop offset="100%" stop-color="#90a4ae"/></linearGradient><linearGradient id="dThermMercury" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#ff8a65"/><stop offset="60%" stop-color="#f4511e"/><stop offset="100%" stop-color="#bf360c"/></linearGradient><radialGradient id="dThermBulb" cx="40%" cy="35%" r="60%"><stop offset="0%" stop-color="#ff8a65"/><stop offset="50%" stop-color="#f4511e"/><stop offset="100%" stop-color="#bf360c"/></radialGradient><radialGradient id="dThermSpec" cx="35%" cy="28%" r="40%"><stop offset="0%" stop-color="rgba(255,255,255,.7)"/><stop offset="100%" stop-color="rgba(255,255,255,0)"/></radialGradient><filter id="dThermBlur"><feGaussianBlur stdDeviation="1"/></filter></defs><ellipse cx="20" cy="37" rx="6" ry="2.2" fill="rgba(0,0,0,.18)" filter="url(#dThermBlur)"/><rect x="16" y="5" width="8" height="22" rx="4" fill="url(#dThermTube)"/><rect x="17.5" y="6" width="2" height="20" rx="1" fill="rgba(255,255,255,.55)"/><rect x="18" y="12" width="4" height="15" rx="2" fill="url(#dThermMercury)"/><circle cx="20" cy="31" r="6" fill="url(#dThermBulb)"/><circle cx="20" cy="31" r="6" fill="url(#dThermSpec)"/><line x1="24.5" y1="10" x2="26.5" y2="10" stroke="rgba(255,255,255,.7)" stroke-width="1" stroke-linecap="round"/><line x1="24.5" y1="15" x2="26.5" y2="15" stroke="rgba(255,255,255,.7)" stroke-width="1" stroke-linecap="round"/><line x1="24.5" y1="20" x2="26.5" y2="20" stroke="rgba(255,255,255,.7)" stroke-width="1" stroke-linecap="round"/><line x1="24.5" y1="25" x2="26.5" y2="25" stroke="rgba(255,255,255,.7)" stroke-width="1" stroke-linecap="round"/></svg></div><div class="stat-info"><div class="stat-val"><?php echo round($weather['heat_index'],1); ?>°C</div><div class="stat-label">Heat Index</div></div></div></div></div><?php else: ?><p style="font-size:.82rem;color:#888;text-align:center;padding:1rem 0;">Walang available na datos ng panahon.</p><?php endif; ?></div><div class="desktop-card"><div class="desktop-card-header"><h2>Evacuate</h2></div><div class="desktop-evac-body"><p style="font-size:.82rem;color:var(--text-2);margin-bottom:.9rem;line-height:1.65;">Kapag available, hanapin ang pinakamalapit na evacuation center at mag-navigate mula sa iyong lokasyon.</p><a href="navigation.php" class="btn-nav">Open Navigation</a></div></div></div><div class="desktop-col-right"><div class="desktop-card"><div class="desktop-card-header" id="announcements"><h2>Announcements</h2></div><?php if (!$announcements): ?><div class="ann-empty">Wala pang anunsyo.</div><?php else: ?><div class="desktop-ann-list ann-list"><?php foreach ($announcements as $idx => $a): ?><div class="ann-item" onclick="openAnnModal(<?php echo $idx; ?>)" role="button" aria-label="Basahin ang anunsyo"><div class="ann-dot <?php echo $a['is_pinned']?'pinned':''; ?>"></div><div class="ann-body"><div class="ann-title"><?php if($a['is_pinned']): ?><span class="badge">NAKA-PIN</span><?php endif; ?><?php if($a['disaster_title']): ?><span class="badge badge-disaster"><?php echo htmlspecialchars($a['disaster_title']); ?></span><?php endif; ?><?php echo htmlspecialchars($a['title']); ?></div><div class="ann-preview"><?php echo htmlspecialchars(mb_substr($a['body'],0,200)); ?></div></div><svg viewBox="0 0 24 24" style="width:15px;height:15px;fill:var(--muted);flex-shrink:0;margin-top:3px;opacity:.5"><path d="M8.59 16.59L13.17 12 8.59 7.41 10 6l6 6-6 6z"/></svg></div><?php endforeach; ?></div><?php endif; ?></div></div></div></div></div>

<script>
'use strict';

// ======================== ANNOUNCEMENT DATA (from PHP) ========================
const ANNOUNCEMENTS_DATA = <?php echo $announcementsJson; ?>;

// ======================== ANNOUNCEMENT MODAL ========================
function openAnnModal(index) {
  const ann = ANNOUNCEMENTS_DATA[index];
  if (!ann) return;

  const SVG_MEGAPHONE = `<svg viewBox="0 0 24 24"><path d="M18 11v2h4v-2h-4zm-2 6.61c.96.71 2.21 1.65 3.2 2.39.4-.53.8-1.07 1.2-1.6-.99-.74-2.24-1.68-3.2-2.4-.4.54-.8 1.08-1.2 1.61zM20.4 5.6c-.4-.53-.8-1.07-1.2-1.6-.99.74-2.24 1.68-3.2 2.4.4.53.8 1.07 1.2 1.6.96-.72 2.21-1.65 3.2-2.4zM4 9c-1.1 0-2 .9-2 2v2c0 1.1.9 2 2 2h1v4h2v-4h1l5 3V6L8 9H4zm11.5 3c0-1.33-.58-2.53-1.5-3.35v6.69c.92-.81 1.5-2.01 1.5-3.34z"/></svg>`;
  const SVG_PIN       = `<svg viewBox="0 0 24 24"><path d="M16 12V4h1V2H7v2h1v8l-2 2v2h5.2v6h1.6v-6H18v-2l-2-2z"/></svg>`;
  const SVG_WARNING   = `<svg viewBox="0 0 24 24"><path d="M1 21h22L12 2 1 21zm12-3h-2v-2h2v2zm0-4h-2v-4h2v4z"/></svg>`;
  const iconSvg = ann.is_pinned ? SVG_PIN : (ann.disaster_title ? SVG_WARNING : SVG_MEGAPHONE);
  document.getElementById('annModalIcon').innerHTML = iconSvg;

  document.getElementById('annModalTitle').textContent = ann.title || 'Anunsyo';

  const metaWrap = document.getElementById('annModalMeta');
  metaWrap.innerHTML = '';

  const chipSvgPin  = `<svg viewBox="0 0 24 24"><path d="M16 12V4h1V2H7v2h1v8l-2 2v2h5.2v6h1.6v-6H18v-2l-2-2z"/></svg>`;
  const chipSvgWarn = `<svg viewBox="0 0 24 24"><path d="M1 21h22L12 2 1 21zm12-3h-2v-2h2v2zm0-4h-2v-4h2v4z"/></svg>`;
  const chipSvgCal  = `<svg viewBox="0 0 24 24"><path d="M20 3h-1V1h-2v2H7V1H5v2H4C2.9 3 2 3.9 2 5v16c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 18H4V8h16v13z"/></svg>`;

  if (ann.is_pinned) {
    const chip = document.createElement('div');
    chip.className = 'annmodal-meta-chip pinned';
    chip.innerHTML = chipSvgPin + ' Naka-Pin';
    metaWrap.appendChild(chip);
  }
  if (ann.disaster_title) {
    const chip = document.createElement('div');
    chip.className = 'annmodal-meta-chip disaster';
    chip.innerHTML = chipSvgWarn + ' ' + escHtml(ann.disaster_title);
    metaWrap.appendChild(chip);
  }
  if (ann.published_at) {
    const chip = document.createElement('div');
    chip.className = 'annmodal-meta-chip';
    let dateStr = ann.published_at;
    try {
      const d = new Date(ann.published_at);
      dateStr = d.toLocaleDateString('fil-PH', { year: 'numeric', month: 'short', day: 'numeric' });
    } catch(_) {}
    chip.innerHTML = chipSvgCal + ' ' + escHtml(dateStr);
    metaWrap.appendChild(chip);
  }
  ...
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      ...
      const overflows = body.scrollHeight > body.clientHeight + 12;
      hint.classList.toggle('visible', overflows);
      body.addEventListener('scroll', hideAnnScrollHint, { once: true, passive: true });
    });
  });
...
function hideAnnScrollHint() {
  document.getElementById('annModalScrollHint').classList.remove('visible');
}
...
function openReadyBagModal() {
  ...
  if (items.length > 1) {
    wrap.innerHTML = items.map((item, i) =>
      `<div class="rbmodal-item" onclick="toggleCheck(${i})" data-idx="${i}">` +
        `<div class="rbmodal-checkbox" id="rbchk-${i}"></div>` +
        `<div class="rbmodal-item-text" id="rbtxt-${i}">${escHtml(item)}</div>` +
      `</div>`
    ).join('');
  } else {
...
function toggleCheck(index) {
  ...
  if (chk) {
    if (isChecked) chk.classList.add('checked');
    else chk.classList.remove('checked');
  }

  const txt = document.getElementById('rbtxt-' + index);
  if (txt) {
    if (isChecked) txt.classList.add('checked-text');
    else txt.classList.remove('checked-text');
  }
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const annSheet = document.getElementById('annModalSheet');
  if (annSheet) {
    let annStartY = 0;
    annSheet.addEventListener('touchstart', e => { annStartY = e.touches[0].clientY; }, { passive: true });
    annSheet.addEventListener('touchend', e => {
      const dy = e.changedTouches[0].clientY - annStartY;
      if (dy > 80) closeAnnModal();
    }, { passive: true });
  }

  const items = document.querySelectorAll('.anim-in');
  items.forEach((el, i) => {
    el.style.animationDelay = (0.04 + i * 0.07) + 's';
    el.style.opacity = '0';
    requestAnimationFrame(() => { el.style.opacity = ''; });
  });

  const wxCard = document.querySelector('.weather-card');
  if (wxCard) {
    wxCard.addEventListener('mousemove', (e) => {
      const rect = wxCard.getBoundingClientRect();
      const x = (e.clientX - rect.left) / rect.width - 0.5;
      const y = (e.clientY - rect.top) / rect.height - 0.5;
      wxCard.style.transform = `perspective(800px) rotateY(${x*3}deg) rotateX(${-y*3}deg) translateY(-2px)`;
    });
    wxCard.addEventListener('mouseleave', () => {
      wxCard.style.transform = '';
      wxCard.style.transition = 'transform .4s cubic-bezier(.22,.68,0,1.18)';
      setTimeout(() => { wxCard.style.transition = ''; }, 400);
    });
  }
});
</script>
