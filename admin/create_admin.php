...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!preg_match('/^09[0-9]{9}$/', $contact))      $errors[] = 'Enter a valid Philippine mobile number (e.g., 09123456789).';
    if (strlen($password) < 8)                        $errors[] = 'Password must be at least 8 characters.';
    if ($password !== $confirm)                        $errors[] = 'Passwords do not match.';
    if (!$barangayId)                                  $errors[] = 'Please select a barangay.';
    if ($houseNo === '')                               $errors[] = 'House/Office number is required.';

    if (!$errors) {
        $chk = $pdo->prepare("SELECT id FROM barangays WHERE id = ? AND is_active = 1");
        $chk->execute([$barangayId]);
        if (!$chk->fetch()) $errors[] = 'Selected barangay is not valid.';
    }
    ...
    if (!$errors) {
        ...
        $stmt = $pdo->prepare("
            INSERT INTO users
                (full_name, email, contact_number, password_hash, role,
                 barangay_id, house_number, is_email_verified, otp_code_hash, otp_expires_at, is_active)
            VALUES (?, ?, ?, ?, 'admin', ?, ?, 1, NULL, NULL, ?)
        ");
        try {
            $stmt->execute([$fullName, $email, $contact, $hash, $barangayId, $houseNo, $isActive]);
            header('Location: users.php?created=admin');
            exit;
        } catch (Throwable $e) {
...
<head>
    <meta charset="UTF-8">
    <title>Create Admin | MDRRMO San Ildefonso</title>
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" />
    <!-- Reuse the same CSS as create_coordinator — identical layout -->
    <link rel="stylesheet" href="../asset/css/admin_create_coordinator.css" />
    <style>
        /* Purple accent for admin creation to visually distinguish from coordinator */
        .btn-submit {
            background: linear-gradient(135deg, #7C3AED, #6D28D9) !important;
        }
        .btn-submit:hover { background: #5B21B6 !important; }
        ...
        .admin-creation-notice i { color: #7C3AED; font-size: 16px; flex-shrink: 0; margin-top: 2px; }
        .admin-creation-notice p { font-size: 13px; color: #4C1D95; line-height: 1.6; margin: 0; }
        .admin-creation-notice strong { display: block; font-size: 13px; font-weight: 700; margin-bottom: 2px; }
    </style>
</head>
<body>
    ...
    <main class="main-content" id="mainContent">
        ...
        <div class="dashboard">
            <div class="page-header">
                <div class="page-header-left">
                    ...
                    <p>
                        ...
                        Create a new MDRRMO administrator account
                    </p>
                </div>
                <a href="users.php" class="btn-cancel" style="text-decoration:none;">
            ...
            <div class="card">
                ...
                <!-- Notice banner -->
                <div class="admin-creation-notice">
                    <i class="fas fa-shield-alt"></i>
                    <p>
                        <strong>Administrator Access</strong>
                        This account will have full access to the MDRRMO system — including user management, disaster records, and evacuation data. Only create admin accounts for trusted MDRRMO personnel.
                    </p>
                </div>
                    ...
                    <div class="error-messages">
                        <ul>
                            <?php foreach ($errors as $err): ?>
                                <li><i class="fas fa-exclamation-circle"></i> <?php echo htmlspecialchars($err); ?></li>
                            <?php endforeach; ?>
                        </ul>
                    </div>
                <?php endif; ?>

                <?php if (isset($_GET['created']) && $_GET['created'] === 'admin'): ?>
                    <div style="padding:12px 16px;background:#E8F5E9;border:1px solid #A5D6A7;border-radius:8px;color:#2E7D32;font-size:13px;margin-bottom:16px;">
                        <i class="fas fa-check-circle"></i> Admin account created successfully.
                    </div>
                <?php endif; ?>
                ...
                <form method="post" class="form">
                    ...
                    <div class="form-group">
                        ...
                        <div class="form-hint">
                            <i class="fas fa-info-circle"></i> Philippine mobile number
                        </div>
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            <label class="form-label">
                                ...
                                Barangay / Office Location <span class="form-required">*</span>
                            </label>
                        ...
                        <div class="form-group">
                            <label class="form-label">
                                ...
                                House / Office Number <span class="form-required">*</span>
                            </label>
...
<script>
    ...
    const pwc = document.querySelector('input[name="confirm_password"]');
    function validatePw() {
        pwc.setCustomValidity(pw.value !== pwc.value ? "Passwords don't match" : '');
    }
    pw.addEventListener('change', validatePw);
